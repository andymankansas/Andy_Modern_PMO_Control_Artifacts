...
if (-not $Destination) {
    $Destination = Join-Path $parent "Walking_Deck_Agent_Setup_$Version.zip"
}
$Destination = [System.IO.Path]::GetFullPath($Destination)
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) "walking-deck-agent-$([guid]::NewGuid())"
$releaseRoot = Join-Path $stagingRoot "Walking_Deck_Agent_Setup_$Version"

try {
    New-Item -ItemType Directory -Path $releaseRoot -Force | Out-Null
    $excludeDirectories = @('.venv', '.pytest_cache', '__pycache__')
    $excludeFiles = @('walking-deck-config.json', 'walking-deck-content.json')
    ...
    Get-ChildItem -LiteralPath $releaseRoot -Recurse -File -Force |
        Where-Object {
            $_.Name -in $excludeFiles -or
            $_.Name -match '_Walking_Deck_.*\.pptx$' -or
            $_.Name -eq 'intake_manifest.json'
        } |
        Remove-Item -Force
